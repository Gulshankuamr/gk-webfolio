...
const BentoGrid: React.FC<{ className?: string; children: React.ReactNode }> = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 md:grid-cols-12 gap-4 p-4 mx-auto max-w-7xl",
        className
      )}
    >
      {children}
...
// UPDATED BentoCard with Hover Effects
const BentoCard: React.FC<any> = ({
  title,
  subtitle,
  description,
  icon: Icon,
  ...
  return (
    <div
      className={cn(
        `group relative overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900 
      ...
      <div className="h-full p-6 flex flex-col justify-between">
        ...
        <div className="flex items-start">
          ...
          <div>
            {title && (
              <h3
                className="font-medium text-zinc-200 text-lg group-hover:text-purple-400 transition-all"
              >
                {title}
              </h3>
            )}
        ...
        {email && (
          <div className="mt-4">
            <a
              href={`mailto:${email}`}
              className="inline-flex items-center justify-center rounded-md bg-zinc-800 
                         px-4 py-2 text-sm font-medium text-zinc-200 hover:bg-zinc-700"
            >
              <svg
...
const featureCards = [
  {
    ...
    description:
      "Building responsive and interactive interfaces using React, Next.js, and Tailwind CSS.",
    icon: <FaReact className="text-blue-400 w-4 h-4 mr-2" />,
  },
  {
    title: "Backend Development",
    description:
      "Creating REST APIs and server-side logic with Node.js, Express, and MongoDB.",
    icon: <FaNodeJs className="text-green-500 w-4 h-4 mr-2" />,
  },
  {
    title: "Fullstack Projects",
    description:
      "Developing end-to-end applications to integrate frontend and backend seamlessly.",
    icon: <FaDatabase className="text-yellow-400 w-4 h-4 mr-2" />,
  },
  {
    title: "UI/UX Design",
    description:
      "Designing clean, user-friendly, and intuitive interfaces for better user experience.",
    icon: <FaPalette className="text-pink-400 w-4 h-4 mr-2" />,
  },
  {
    title: "Learning & Experimentation",
    description:
      "Exploring new technologies, practicing coding, and implementing modern development techniques.",
    icon: <FaLightbulb className="text-yellow-300 w-4 h-4 mr-2" />,
  },
  {
    title: "Version Control & Collaboration",
    description:
      "Managing projects efficiently using Git and GitHub while collaborating on code.",
